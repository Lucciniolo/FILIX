...
FILIX = True # activate a filter that erase forwading and hide email adress to prevent from spam
...
class MessageFeed :
  ...
  def add_message(self, message) :
    ...
    if FILIX: # filix in action
        originalTitle = title_text
        title_text = filix.changeTitle(title_text)
    ...
    if contents :
      ...
      contents.sort(lambda x, y : cmp(y,x))  # x, y : HTML prefered, y, x : TEXT
      if FILIX: # filix in action
        (content_type,content_text) = filix.doIt(contents, originalTitle) 
        #(content_type,content_text) = contents[0]
        print "DEBUG : le titre du mail :\n", title_text
        print "\n \n"
        print "DEBUG : Le contenu du mail :\n", content_text
      else: 
        (content_type,content_text) = contents[0]
...
if __name__ == "__main__" :
  ...
  for message in source.messages() :
    # filix.doIt(FILIX, message) # FILIX in action
    # message.set_payload("areuhareuh")
    # print message
